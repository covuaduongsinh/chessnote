...
**Note:** Exports are one-offs. If you are interested in sharing content onto other platforms and keeping things in sync, have a look at [[Features/Share]].
...
* PDF: renders the page to a downloadable PDF file — 2-column layout, automatic page numbers, and chess `fen`/`pgn`/`puzzle` blocks as static board images. Works the same on the web app and the desktop app (each uses its own headless Chrome to render it).
